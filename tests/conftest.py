...
def make_session(request, url: str):
    """Connect a session to a URL."""
    sess = qi.Session()
    sess.connect(url)
...
def process(request, exec_path):
    ...
    proc = subprocess.Popen(
        exec_path.split(" "), stdout=subprocess.PIPE, universal_newlines=True
    )
...
def process_service_endpoint(process):
    """Endpoint of a service with its name that a process returns on
    its standard output.
    """
    ...
    for line in map(str.strip, iter(process.stdout.readline, "")):
...
def pytest_generate_tests(metafunc):
    ...
    if "exec_path" in metafunc.fixturenames:
        metafunc.parametrize(
            "exec_path", metafunc.config.getoption("exec_path")
        )
